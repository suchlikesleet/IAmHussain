...
namespace BOH
{
    public class TriggerSystem : MonoBehaviour
    {
        ...
        private void CheckAmbientTriggers()
        {
            ...
            foreach (var trigger in allTriggers.Where(t => t.type == TriggerSO.TriggerType.Ambient))
            {
                if (CanTrigger(trigger) && hour >= trigger.startHour && hour < trigger.endHour)
                {
                    // For ambient triggers, you might want to check player position
                    // or other conditions. For now, trigger when time is right
        ...
        private bool CanTrigger(TriggerSO trigger)
        {
            ...
            // Check trust level if needed
            // (Would need ContactSystem reference for this)
    ...
    // ScriptRole: Manages trigger conditions and offers errands
    // RelatedScripts: ErrandSystem, OfferPrompt, TimeSystem
    // UsesSO: TriggerSO, ErrandSO, GameEventSO
    // ReceivesFrom: TimeSystem, GameStateManager
    // SendsTo: ErrandSystem, OfferPrompt UI
}